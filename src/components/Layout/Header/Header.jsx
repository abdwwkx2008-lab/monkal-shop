...
function Header() {
  const { products, cart, favorites, theme, toggleTheme } =
  ...
  const mainNavLinks = [
    ...
    },
    {
      ...
      path: '/odejdacatalog',
      type: 'subcategory',
      sublinks: clothingSubcategories,
    },
    {
      name: 'Обувь',
      path: '/shoescatalog',
      type: 'subcategory',
      sublinks: shoesSubcategories,
    },
    {
      name: 'Аксессуары',
      path: '/aksesuarycatalog',
      type: 'subcategory',
      sublinks: accessoriesSubcategories,
    },
    {
      name: 'Бренды',
      path: '/brands',
      type: 'brands',
      sublinks: menuData['Бренды'] || [],
    },
  ...
  return (
    <>
      <header className="header">
        <div className="header-container">
          <div className="header-group-left">
            ...
            <button
              className="header-icon-btn search-mobile"
          ...
          <div className="header-group-right">
            <button
              className="header-icon-btn search-desktop"
              onClick={() => setIsSearchOpen(true)}
            >
              <SearchIcon color={iconColor} />
            </button>
            <Link to="/favorites" className="header-icon-btn">
              <div className="icon-wrapper">
                ...
                {totalFavoritesItems > 0 && (
                  <span className="icon-counter">{totalFavoritesItems}</span>
                )}
              </div>
            </Link>
            <Link to="/basket" className="header-icon-btn">
              <div className="icon-wrapper">
                ...
                {totalCartItems > 0 && (
                  <span className="icon-counter">{totalCartItems}</span>
                )}
              </div>
            </Link>
      ...
      <nav className={`mobile-nav ${isMobileMenuOpen ? 'open' : ''}`}>
        <Link
          to="/"
          className="mobile-nav-logo-wrapper"
