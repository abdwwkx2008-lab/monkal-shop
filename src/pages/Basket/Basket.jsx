...
const Basket = () => {
  ...
  const decrementCount = (cartId) => {
    setCart((prevCart) =>
      prevCart.map((item) =>
        getUniqueCartId(item) === cartId && item.count > 1
          ? { ...item, count: item.count - 1 }
          : item
      )
    )
  ...
  const removeCartItem = (cartId) => {
    setCart((prevCart) =>
      prevCart.filter((item) => getUniqueCartId(item) !== cartId)
    )
  }
    ...
    return (
      <div className="empty-basket-container">
        <div className="empty-basket-content">
          <svg className="empty-basket-icon" viewBox="0 0 24 24">
            <path d="M19.5,8H17.21l-3.3-6.6a.75.75,0,0,0-1.34.66l3,6h-7.14l3-6A.75.75,0,0,0,10.13,1.4L6.79,8H4.5a.75.75,0,0,0,0,1.5h.34l.89,7.12A4,4,0,0,0,9.66,20H14.3a4,4,0,0,0,3.93-3.38L19.16,9.5h.34a.75.75,0,0,0,0-1.5Zm-4.34,9.62A2.5,2.5,0,0,1,12.72,18.5H11.24a2.5,2.5,0,0,1-2.43-2.12L8,9.5h8Z" />
          </svg>
          <h2 className="empty-basket-title">Ваша корзина пуста</h2>
          <p className="empty-basket-text">
            Добавьте что-нибудь, чтобы сделать её счастливой!
          </p>
          <button
      ...
      <section className="basket-section">
        <div className="basket-container">
          <div className="basket-header">
            ...
            <button onClick={clearCart} className="clear-basket-button">
              Очистить корзину
            </button>
          </div>
          <div className="basket-layout">
                  ...
                  <div key={uniqueId} className="basket-item-card">
                    ...
                    <div className="item-details">
                      ...
                      <div className="item-quantity-stepper">
                        <button onClick={() => decrementCount(uniqueId)}>
                          -
                        </button>
                        <span>{item.count}</span>
                        <button onClick={() => incrementCount(uniqueId)}>
                          +
                        </button>
                      </div>
                    </div>
                    <div className="item-price-and-remove">
                      <p className="item-price">
                        {(item.price * item.count).toLocaleString()} С
                      </p>
                      <button
            ...
            <div className="order-summary-card">
              ...
              <button onClick={handleCheckout} className="checkout-main-button">
      ...
      {isCheckoutModalOpen && (
        <div className="logout-modal-overlay">
          <div className="logout-modal-content">
            ...
            <p>
              Ваш заказ на сумму {totalPrice.toLocaleString()} С будет оформлен.
              Продолжить?
            </p>
            <div className="logout-modal-actions">
              ...
              <button onClick={confirmOrder} className="modal-btn-confirm">
                Подтвердить
              </button>
            </div>
          </div>
