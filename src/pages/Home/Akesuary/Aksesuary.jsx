...
const Aksesuary = () => {
    ...
    const accessories = products.filter(item => item.category === "аксессуары").slice(0, 6);
            ...
            <div className="container">
                <div className="section-header">
                    ...
                    <Link to='/aksesuarycatalog' className="section-header-link">
                        ...
                        <svg width="24" height="24" viewBox="0 0 24 24"><path d="M16.172 11l-5.364-5.364 1.414-1.414L20 12l-7.778 7.778-1.414-1.414L16.172 13H4v-2z"/></svg>
                    </Link>
                </div>
                    ...
                    {accessories.map(function(item) {
                        return (
                            <SwiperSlide key={item.id} className="shoe-slide">
                                <div className="shoe-card" onClick={() => navigate(`/product/${item.id}`)}>
                                    <div className="shoe-image-wrapper">
                                        <img className="shoe-img" src={item.image} alt={item.name} />
                                    </div>
                                    <div className="shoe-info-wrapper">
                                        <p className="shoe-brand">{item.brand}</p>
                                        <p className="shoe-name">{item.name}</p>
                                        <p className="shoe-price">{item.price.toLocaleString()} ₽</p>
                                    </div>
                                </div>
                            </SwiperSlide>
                        );
