...
const FRONTEND_URL = process.env.FRONTEND_URL || 'https://monkal-shopp.vercel.app';
...
app.post('/login', async (req, res) => {
    ...
    if (error) return res.status(500).json({ message: error.message });
    if (!user) return res.status(400).json({ message: 'Неверный логин или пароль' }); // ⚠️ Здесь была проверка на is_verified
...
console.log('[ENV]', process.env.GMAIL_USER, process.env.JWT_SECRET);
